...
import { ArrowDown, ArrowUpRight, Aperture, BrainCircuit, ChevronLeft, ChevronRight, CircleDot, Compass, Cpu, Mail, Moon, Mountain, Route, Sun, X } from 'lucide-react'
...
const photos = [
  { src: '/photos/gallery-01.jpg', title: 'Observation / 01', note: '光影记录' },
  { src: '/photos/gallery-02.jpg', title: 'Observation / 02', note: '光影记录' },
  { src: '/photos/gallery-03.jpg', title: 'Observation / 03', note: '光影记录' },
  { src: '/photos/gallery-04.jpg', title: 'Observation / 04', note: '光影记录' },
  { src: '/photos/gallery-05.jpg', title: 'Observation / 05', note: '光影记录' },
  { src: '/photos/gallery-06.jpg', title: 'Observation / 06', note: '光影记录' },
]

const interestDetails = {
  go: { label: 'GO · AMATEUR 2 DAN', title: '围棋：在局部之外看全局', text: '围棋训练我在复杂局面中计算与判断：什么时候继续争取，什么时候接受取舍，又如何让局部选择服务于全局。业余二段并不意味着职业水平，它更像一段长期练习的记录。它提醒我，耐心和复盘往往比一时的得失更重要。' },
...
function LightTrail() {
  ...
  useEffect(() => {
    ...
    const tween = gsap.to(dot.current,{y:()=>document.body.scrollHeight-innerHeight-180,ease:'none',scrollTrigger:{trigger:document.body,start:'top top',end:'bottom bottom',scrub:1}})
    return () => { tween.kill() }
  },[])
...
function MagneticLink({href, children}:{href:string, children:React.ReactNode}) {
  ...
  return <motion.a href={href} className="magnetic" style={{x:sx,y:sy}} onPointerMove={e=>{const r=e.currentTarget.getBoundingClientRect();x.set((e.clientX-r.left-r.width/2)*.12);y.set((e.clientY-r.top-r.height/2)*.12)}} onPointerLeave={()=>{x.set(0);y.set(0)}}>{children}</motion.a>
}
function TiltCard({children,className=''}:{children:React.ReactNode,className?:string}) {
  const x=useMotionValue(0), y=useMotionValue(0)
  const rx=useSpring(useTransform(y,[-.5,.5],[7,-7]),{stiffness:180,damping:18})
  const ry=useSpring(useTransform(x,[-.5,.5],[-7,7]),{stiffness:180,damping:18})
  return <motion.div className={`tilt ${className}`} style={{rotateX:rx,rotateY:ry,transformStyle:'preserve-3d'}} onMouseMove={e=>{const r=e.currentTarget.getBoundingClientRect();x.set((e.clientX-r.left)/r.width-.5);y.set((e.clientY-r.top)/r.height-.5)}} onMouseLeave={()=>{x.set(0);y.set(0)}}>{children}</motion.div>
}

function SectionHead({number,eyebrow,title,desc}:{number:string,eyebrow:string,title:string,desc?:string}) {
  return <div className="section-head reveal"><span>{number} / {eyebrow}</span><h2>{title}</h2>{desc&&<p>{desc}</p>}</div>
}

function CubeVisual({transition=false}:{transition?:boolean}) {
  const faces=['front','back','right','left','top','bottom']
  return <div className={transition?'cube-auto transition-cube-object':'cube-auto'}>{faces.map(face=><div className={`cube-face ${face}`} key={face}>{Array.from({length:9},(_,i)=><i key={i}/>)}</div>)}</div>
}
...
function Metric({value,label,note}:{value:string,label:string,note:string}) {
  return <article className="metric reveal"><span>{label}</span><strong>{value}</strong><p>{note}</p></article>
}
...
function App() {
  ...
  return <main>
    ...
    <section className="about old-section" id="about">
      <div className="old-container"><header className="old-section-head reveal"><h2>用逻辑理解问题，<br/>也为感受留下空间。</h2></header><div className="old-about-layout"><div className="old-about-copy reveal"><p>我毕业于深圳红岭中学，现在是南方科技大学的一名本科新生。<strong>数学训练让我习惯拆解问题、寻找秩序</strong>；摄影则提醒我，世界并不只由结论构成，也有光线、情绪与稍纵即逝的瞬间。</p><p>站在大学的起点，我对电子信息、计算机与机器人保持兴趣，但并不急着为自己限定唯一方向。比起过早给出答案，我更愿意先打牢基础，在课程、编程学习和未来的科研实践中认真体验。</p><p>我并非总能立刻找到最好的路径，但愿意投入时间，在判断、修正与继续前进之间保持耐心。</p><div className="old-trait-list"><div>逻辑与分析<span>Logic</span></div><div>观察与感受<span>Observation</span></div><div>专注与耐心<span>Focus</span></div><div>开放与好奇<span>Curiosity</span></div></div></div><div className="old-about-side reveal"><dl className="old-profile-facts"><div><dt>现在</dt><dd>南方科技大学 2026 级本科新生</dd></div><div><dt>来自</dt><dd>深圳红岭中学</dd></div><div><dt>关注</dt><dd>数学基础、科技探索与长期成长</dd></div><div><dt>镜头</dt><dd>晚霞、云层与城市边缘</dd></div></dl><div className="old-education-path"><div className="old-school"><img src="/hongling-logo.png" alt="深圳红岭中学校徽"/><strong>深圳红岭中学</strong><small>Foundations · High School</small></div><div className="old-school-route"><i/><span>A new chapter unfolds</span></div><div className="old-school"><img src="/sustech-logo.png" alt="南方科技大学校徽"/><strong>南方科技大学</strong><small>Now · University</small></div><p>从一段扎实的高中积累，走向仍在展开的大学起点。</p></div></div></div></div>
    </section>

    <section className="journey old-section" id="journey"><div className="old-container"><header className="old-section-head reveal"><h2>一些被认真投入<br/>留下的坐标。</h2><span>荣誉不是终点，更像是在不同阶段对专注与积累的记录。</span></header><div className="old-metric-grid reveal"><div className="achievement-origin" aria-hidden="true"><i/></div><article><small>MATHEMATICS</small><strong>TOP</strong><h3>多次数学年级第一</h3><p>深耕数理思维，具备扎实数学功底。</p></article><article><small>HONGLING STAR</small><strong>STAR</strong><h3>“红岭之星”</h3><p>全年级仅四人获得。</p></article><article><small>MATH COMPETITION</small><strong>1st Prize</strong><h3>校长杯数学竞赛一等奖</h3><p>一段关于数学训练的认真记录。</p></article><article><small>GO</small><strong>2 DAN</strong><h3>围棋业余二段</h3><p>在计算、取舍与耐心中学习。</p></article></div><div className="old-timeline"><article className="reveal"><i/><span>FOUNDATIONS</span><div><h3>红岭阶段：建立学习的基础</h3><p>在深圳红岭中学持续训练数学思维，多次取得数学年级第一，并获得“红岭之星”与校长杯数学竞赛一等奖。</p></div></article><article className="reveal"><i/><span>BEYOND STUDY</span><div><h3>学习之外：观察与耐心</h3><p>风光摄影作品曾在 500px 发布并被平台收录；围棋业余二段，则记录着计算、判断和长期练习。</p></div></article><article className="reveal"><i/><span>NOW</span><div><h3>大学起点：方向仍在展开</h3><p>进入南方科技大学后，继续夯实基础，并在电子信息、计算机与机器人等方向之间保持开放探索。</p></div></article></div></div></section>
    ...
    <section className="gallery-wrap" ref={gallery}><div className="gallery-track" ref={track}><div className="gallery-lead"><span>SELECTED<br/>OBSERVATIONS</span><em className="gallery-credit">个人作品集 · 均为本人拍摄</em><p>六个瞬间<br/>关于光、时间与空间</p></div>{photos.map((p,i)=><figure key={p.src} className={`photo-card p${i % 3}`} role="button" tabIndex={0} aria-label={`全屏查看 ${p.title}`} onClick={()=>setActivePhoto(i)} onKeyDown={e=>{if(e.key==="Enter")setActivePhoto(i)}}><img src={p.src} alt={`${p.title}，${p.note}`} loading="lazy"/><figcaption><span>{p.title}</span><p>{p.note}</p></figcaption></figure>)}</div></section>
