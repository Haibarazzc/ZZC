...
const SAMPLE = '/photos/gallery-03.jpg'
...
export default function SunsetColorLab({language,theme,onClose,onToggleLanguage,onToggleTheme}:{language:Language;theme:Theme;onClose:()=>void;onToggleLanguage:()=>void;onToggleTheme:()=>void}) {
  const [source,setSource]=useState(SAMPLE),[filename,setFilename]=useState('gallery-03.jpg'),[analysis,setAnalysis]=useState<Analysis|null>(null),[status,setStatus]=useState<'loading'|'ready'|'error'>('loading')
  ...
  const reset=()=>{if(objectUrl.current)URL.revokeObjectURL(objectUrl.current);objectUrl.current=null;setFilename('gallery-03.jpg');setSource(SAMPLE)}
