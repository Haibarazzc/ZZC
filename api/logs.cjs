// Vercel serverless function: /api/logs?key=...&date=YYYY-MM-DD (CommonJS)
...
function beijingDate(offsetDays) { offsetDays = offsetDays || 0; return new Date(Date.now() + (8 + offsetDays * 24) * 3600 * 1000).toISOString().slice(0, 10) }
...
module.exports = async function handler(req, res) {
  ...
  try {
    ...
    events = (data.result || []).map((s) => { try { return JSON.parse(s) } catch { return null } }).filter(Boolean)
  } catch { events = [] }
